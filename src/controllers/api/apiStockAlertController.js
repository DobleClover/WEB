...
import sendAdminStockNotificationEmail from "../../utils/helpers/sendAdminStockNotificationEmail.js";
const { Variation } = db;

const controller = {
  createStockAlert: async (req, res) => {
    try {
      const { products_id, sizes_id, email, phone_number, phone_countries_id } = req.body;
      
      if (!products_id || !sizes_id || !email || !phone_number || !phone_countries_id) {
        return res
          .status(HTTP_STATUS.BAD_REQUEST.code)
          .json({ msg: "Faltan datos obligatorios" });
      }
      let stockAlertObj = {
        id: uuidv4(),
        products_id,
      ...
      await db.StockAlert.create(stockAlertObj);
      await sendAdminStockNotificationEmail(stockAlertObj);
      return res
        .status(HTTP_STATUS.CREATED.code)
        .json({ msg: "Aviso guardado correctamente" });
    } catch(error) {
      console.log("Falle en createStockAlert");
...
export const getStockAlertsFromDB = async (id) => {
  try {
    if (!id) return [];

    return;
  } catch (error) {
    console.log(`Error finding stock alerts in db: ${error}`);
    console.log(error);
    return null;
  }
};
...
export const populateVariations = (variations, colorsFromDB = []) => {
  return variations.map((variation) => {
    ...
    const color = colorsFromDB.find(
      (dbColor) => dbColor.id == variation.colors_id
    );
    return {
      ...variation,
      size: sizePopulated,
